...
const getRate = (req, res) => {
  ...
  const query = "SELECT rate.id , users.email,users.firstName ,rate.rate, rate.user_id FROM rate INNER JOIN users ON rate.user_id=users.id INNER JOIN products ON rate.product_id=products.id WHERE rate.is_deleted=0 AND rate.product_id=?";
...
const updateRate = (req, res) => {
    ...
    const user_id = req.token.userId
    const query = "UPDATE rate SET rate=? WHERE id = ? AND is_deleted=0 AND user_id=?";
